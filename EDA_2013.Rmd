...
2)  Survey design includes stratification alongside random sampling. Use
    of the **final weight** in analysis is necessary if users are to
    make generalizations from the sample to the population. I will skip
    it for now, so **my results cannot be generalized to the
    population**.

3)  Data was collected through a telephone-based survey, so it is
    **non-response biased**. [@schneider2010]

Age and ethnicity misrepresentation. Distribution of the sample by the
calculated age groups (**X_age_g**) and sex (**sex**) filled by race
...
Indeed, age demographics is heavily skewed to the older age, especially
in the female group. Race demographics skewed as well. As this is
crucial for calculating this skewness should be addressed in detailed
analysis later.
...
-   Type 1 diabetes is an idiopathic (fancy word for "we don't know the
    ...
    resulting from failure of the pancreas to produce enough insulin due
...
This is a very personal problem for me as despite maintaining normal
weight and a relatively healthy lifestyle I have to deal with metabolic
syndrome. Keeping a diet low in refined sugars and regular physical
exercises helped me drastically increase life quality.
...
There is a more precise metric - hip to waist ratio, which takes into
...
dataset. Type 1 diabetes is relatively rare compared to type 2 - around
...
As a total diabetes score I'm going to use diabetes, pre-diabetes and
...
As a total CV disease score I'm going to use a combination of these 4
variables.
...
roughly estimate healthy food habits from the amount of fruit portions
per day (only whole fruits, as I do not include fresh juice here because
of its high sugar low fiber content), green and orange vegetable
portions per day (as others can be too starchy) and legumes per week.
...
Excessive caloric intake in the form of a diet high in fast carbs was
stated as one of the major risk factors for developing insulin
resistance. I'm interested in the evaluation of correlation between
sugar consumption and health based on accessible parameters. There isn't
much data available here, as there are just two sugar consumption
questions and they are located in the optional part of the
questionnaire.
...
I would love to use a numerical variable reflecting the amount of
cigarettes smoked a day, but this data isn't available here.
...
development of insulin resistance and diabetes. I will include the
amount of sleep hours as my metric, as there is no other data available
here.
...
analysis which will account for skewness and ethnic differences
...
If the overall difference is not big, there is an increasing gap between
men and women as income level increases. It would be interesting to look
at fresh data, as there has been a new trend in healthy lifestyle
recently. Extensive analysis which will account for skewness and ethnic
differences (different BMI evaluation for Asian) is needed.
...
In the both diagrams similar patterns can be noticed with increased
levels for some groups (Native Hawaiian or other Pacific Islander, Black
or African American and American Indian or Alaskan Native). But due to
serious misrepresentation of these groups it requires adjustments.

Extensive analysis which will account for skewness and weights (account
...
3.1.2), levels of diabetes differ significantly. From which I can make a
conjecture that income level alone could be a significant risk factor
for diabetes.
...
Overall dynamics is the same with diabetes - income level alone could be
a significant risk factor for CV diseases.
...
very weak. CV diseases correlate most heavily with age as expected, then
comes diabetes, BMI and income level.
...
We can see that the combined general health score is distributed more
continuously.
...
**ssbsugar** and **ssbsugar** are from Optional Module, thus there are
not many data records available. So I have to create a separate dataset.
...
About negative correlation and diabetes: it is most likely due to the
observational nature of the study - it is expected behavior from
diabetic to stop consuming sugary drinks and other fast carbs. What is
...
1.  On almost all parameters related to food habits correlation is
    ...
    from an epidemiological observational study, thus it can be the
    other way around - people with health problems switching to a more
    healthy lifestyle in order to mitigate their problem. Low
    correlation of BMI with almost every lifestyle related variable was
    a surprise for me though, it needs additional exploration in the
    future.\
    \
    There are a lot of non-linear relationships which require extensive
    ...
    results could also be due to the observational nature of the study
    (survival bias).
