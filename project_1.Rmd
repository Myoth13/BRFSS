...
1)  BRFSS conducts both landline telephone- and cellular telephone-based
    ...
    interviewers collect data from a randomly dplyr::selected adult in a
    ...
    **Conclusion about inference** - this is observational study - only
    **correlation** can be evaluated.

2)  Data was collected through **random sampling**. BRFSS is an ongoing
    surveillance system designed to measure behavioral risk factors for
    the **non-institutionalized adult population (18 years of age and
    older) residing in the US**. Use of the final weight in analysis is
    necessary if users are to make generalizations from the sample to
    the population.

3)  Data was collected through a telephone-based survey, so it is
    non-response biased. [@schneider2010]

Distribution of the sample by the calculated age groups (**X_age_g**)
and sex (**sex**) filled by employment status (**employ1**):
...
Indeed, demographics is skewed to the older age, especially in the
female group. Analysis will be made separately by age groups to mitigate
this skewness.
...
many new ones as well. One of them is obesity and diseases associated
with it - diabetes type 2, cardiovascular diseases, cancer,
...
and maintaining a relatively healthy lifestyle I still have metabolic
syndrome. Low carb diet and regular physical exercises helped me
drastically increase my quality of life.
...
#### 2.3.7 Obesity correlation with diabetes and pre-diabetes by income levels
...
for_graph_obese_age <- dat_q1 %>%
  filter(!is.na(X_age_g), !is.na(X_rfbmi5),) %>% 
  group_by(X_age_g,sex,X_rfbmi5) %>% 
  summarise(n=n()) %>% 
...
for_graph_obese_age %>%
  ggplot(aes(x = sex, y=proc,  fill = X_rfbmi5 )) +
  labs( 
       x="", 
       y="percentage",
...
Overweight rate is almost persistent in all age groups older 35.
Although there is a noticable difference between sexes.
...
for_graph_obese_inc <- dat_q1 %>%
  filter(!is.na(X_incomg), !is.na(X_rfbmi5),) %>% 
  group_by(X_incomg,sex,X_rfbmi5) %>% 
  summarise(n=n()) %>% 
...
prominent in the middle-old age groups and low income group.

#### 3.1.5 Obesity correlation with diabetes and pre-diabetes
...
dat_q1 %>% 
  filter(!is.na(diabetes_char),!is.na(X_rfbmi5),!is.na(X_incomg)) %>% 
  ggplot() + 
    geom_mosaic(aes(x = product(diabetes_char, X_rfbmi5), fill = diabetes_char )) +
    labs(
      y="Diabetes or pre-diabetes", 
      x="Overweight", 
...
Despite almost the same obesity rate among income groups, levels of
diabetes differ significantly. Proportion of overweight diabetics to
normal weight diabetics stays almost the same among all income groups.

#### 3.1.6 Obesity correlation with cardiovascular (CV) diseases
...
dat_q1 <- dat_q1 %>% 
  ...
  mutate(heart_char = case_when( 
                        as.numeric(cvdinfr4) == 1 ~ "CV", 
                        as.numeric(cvdcrhd4) == 1 ~ "CV",
                        as.numeric(cvdstrk3) == 1 ~ "CV",
                        as.numeric(bphigh4) == 1 ~ "CV",
                        TRUE ~ "No CV" 
  ) %>% factor())    
...
for_graph %>%
   ggplot(aes(x = X_rfbmi5, y = perc, fill=forcats::fct_rev(heart_char), label=perc)) +
      labs( 
      ...
      geom_text(
        aes(x=X_rfbmi5, y=perc, label = percent(perc), vjust=1.5), 
...
Here I added numerical representation of percentages because here we can
see slightly different pattern. Income has a less weight in the obese
(increased in 1,5 times between marginal income levels) than normal
group (percent of CV doubled).
